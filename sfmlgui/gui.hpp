...
inline constexpr sf::Color HOVER_COLOR =
    sf::Color(GUI_PRIMARY_COLOR.r - 20, GUI_PRIMARY_COLOR.g - 20,
              GUI_PRIMARY_COLOR.b - 20);
inline constexpr sf::Color CLICK_COLOR =
    sf::Color(GUI_PRIMARY_COLOR.r + 20, GUI_PRIMARY_COLOR.g + 20,
              GUI_PRIMARY_COLOR.b + 20);
...
public:
  ...
  // Setup, After all widgets are added to the GUI, we need to do an initial
  // setup This should be called after all widgets are added with
  // SFMLGUI::Add();
  void Setup();

  // Update, Runs every frame, handles interactions with window decorations, and
  // calls each widgets SoftUpdate() function
  void Update();
